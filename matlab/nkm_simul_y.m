function [C,N,Y,PI,R,WP,RMC,Y_exp,R_exp,PI_exp,C_exp,RMC_exp,U] = ...
    nkm_simul_y(fspace,B_proc,par,QP,QW,PHI_FG,zlb,stst,hor)
...
RMC     = zeros(T,1);
U       = zeros(T,1);
FG      = zeros(T,1);   FG(1) = stst(4); % Output
Y_exp   = zeros(T,hor);
R_exp   = zeros(T,hor);
PI_exp  = zeros(T,hor);
C_exp   = zeros(T,hor);
...
for t=2:T
    ...
    RMC(t)  = WP(t);
    R(t)    = R_LR*((PI(t)/PI_LR).^PHI_PI).*((Y(t)/Y_LR).^PHI_Y).*((FG(t-1)/Y_LR).^PHI_FG);
    ...
    for j=1:hor
        ...
        R1    = R_LR*((PI1/PI_LR).^PHI_PI).*((Y1/Y_LR).^PHI_Y).*...
            ((F_aux*ones(length(QP),1)/Y_LR).^PHI_FG);

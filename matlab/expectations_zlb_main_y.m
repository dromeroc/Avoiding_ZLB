...
for j=1:length(FGpar)
    ...
    [Hy,Hz,eu] = mysolvemodel(@nk_model_y,stst2,pars,nvar,nshocks,nEerr);
    ...
    fgN     = 20;
    pct     = 0.1;
    fgmin   = log(stst(4)*(1-pct)); % output stst-pct
    fgmax   = log(stst(4)*(1+pct)); % output stst+pct
    ...
    nodes{1}(mflookup(nodes{1},stst(1),3))= stst(1);          % Put ss of B in nodes
    nodes{2}(mflookup(nodes{2},log(stst(4)),3))= log(stst(4));% Put ss of Y in nodes
    Grid    = gridmake(nodes);
    Basis   = funbas(fspace);
    ...
    % variable in the system and output is the fourth variable.)
    c_guess     = stst(2)+Hy(2,1)*(Grid(:,1)-stst(1))+Hy(2,4)*(Grid(:,2)-log(stst(4)));
    pi_guess    = stst(5)+Hy(5,1)*(Grid(:,1)-stst(1))+Hy(5,4)*(Grid(:,2)-log(stst(4)));
    ...
    zlb = 0;
    par1 = broyden('nkm_res_y',guess,fspace,Grid,B1,QW,PHI_FG,zlb);
    
    % Solve the model with the ZLB (use as initial guess previous solution)
    zlb = 1;
    par2 = broyden('nkm_res_y',par1,fspace,Grid,B1,QW,PHI_FG,zlb);
    
    % Recover policy functions without the ZLB
    zlb = 0;
    [~,C1,N1,Y1,PI1,R1,WP1,RMC1] = nkm_res_y(par1,fspace,Grid,B1,QW,PHI_FG,zlb);
    
    % Check Euler residuals without the ZLB
    [EulerRes1,~,~,~,~,~,~,~,CRes1] = nkm_res_y(par1,fspace,Grid_large,...
        B1_large,QW,PHI_FG,zlb);
    
    % Recover policy functions with the ZLB
    zlb = 1;
    [~,C2,N2,Y2,PI2,R2,WP2,RMC2] = nkm_res_y(par2,fspace,Grid,B1,QW,PHI_FG,zlb);
    
    % Check Euler residuals with the ZLB
    [EulerRes2,~,~,~,~,R2_large,~,~,CRes2] = nkm_res_y(par2,fspace,Grid_large,...
        B1_large,QW,PHI_FG,zlb);
...
%     set(groot, 'defaultLegendInterpreter','latex')
    ...
    [C,~,Y,PI,R,~,RMC,Y_exp,R_exp,PI_exp,C_exp,RMC_exp,U] = ...
        nkm_simul_y(fspace,B_proc_irf,par1,QP,QW,PHI_FG,zlb,stst,hor);
    ...
    [Cz,~,Yz,PIz,Rz,~,RMCz,Yz_exp,Rz_exp,PIz_exp,Cz_exp,RMCz_exp,Uz] = ...
        nkm_simul_y(fspace,B_proc_irf,par2,QP,QW,PHI_FG,zlb,stst,hor);
    ...
    [Cs,~,Ys,PIs,Rs,~,RMCs,Ys_exp,Rs_exp,PIs_exp,Cs_exp,RMCs_exp,Us] = ...
        nkm_simul_y(fspace,B_proc_sim,par1,QP,QW,PHI_FG,zlb,stst,hor);
    ...
    [Csz,~,Ysz,PIsz,Rsz,~,RMCsz,Ysz_exp,Rsz_exp,PIsz_exp,Csz_exp,RMCsz_exp,Usz] = ...
        nkm_simul_y(fspace,B_proc_sim,par2,QP,QW,PHI_FG,zlb,stst,hor);

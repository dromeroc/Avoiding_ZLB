function [res,C,N,Y,PI,R,WP,RMC,cres] = nkm_res_p(guess,fspace,Grid,B1,QW,PHI_FG,zlb)
...
WP      = (C.^SIGG).*(N.^PHI);
RMC     = WP;
R       = R_LR*((PI/PI_LR).^PHI_PI).*((Y/Y_LR).^PHI_Y).*((FG/PI_LR).^PHI_FG);
...
C1      = funeval(guess(:,1),fspace,[B1 kron(log(PI),ones(length(QW),1))]);
PI1     = funeval(guess(:,2),fspace,[B1 kron(log(PI),ones(length(QW),1))]);
